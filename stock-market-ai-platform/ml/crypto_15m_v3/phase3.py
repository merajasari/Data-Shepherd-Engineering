...
def _prob(row, state: str) -> float:
    return float(row[f"prob_{state.lower()}"])


def _ret(row, state: str) -> float:
    return float(row[f"{state.lower()}_forward_return_1h"])
...
def simulate(frame: pd.DataFrame, policy: Policy, cost_bps: float) -> tuple[dict, pd.DataFrame]:
        ...
        if raw == state:
            ...
            if state_age < policy.min_hold_bars:
                ...
                if pending_count < policy.confirmation:
                    ...
                    current_expected = _prob(row, state)
                    proposed_expected = _prob(row, raw)
                    net_edge_proxy = proposed_expected - current_expected
                    if net_edge_proxy <= hurdle:
                        blocked_cost += 1
    ...
    peak = detail["equity"].cummax()
    dd = detail["equity"] / peak - 1.0
    summary = {
        "policy_id": policy.policy_id,
        "cost_bps": cost_bps,
...
def main():
    ...
    detail_df.to_csv(DAILY_METRICS, index=False)
    MANIFEST.write_text(json.dumps({
        ...
        "important_limitation": "Probability-margin and cost hurdle are policy proxies; no real order execution or brokerage slippage model is used.",
