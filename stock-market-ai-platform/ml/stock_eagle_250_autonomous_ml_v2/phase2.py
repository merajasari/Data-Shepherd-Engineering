"""StockEagle250 Autonomous ML V2 Phase 2: tail-aware nested walk-forward.

Trains the preregistered alpha, downside, conditional-tail, and meta-allocation
learners. The meta allocator is trained only from inner walk-forward OOS base
...
OUTPUT_ROOT = Path("data/model/stock_eagle_250_autonomous_ml_v2/phase2")
...
def validate_phase1(manifest: dict, contract: dict) -> None:
    ...
    if problems:
        raise RuntimeError(
            "Autonomous ML V2 Phase 2 rejected Phase 1: "
            + ", ".join(problems)
        )
    ...
    for key in (
        ...
        if nested.get(key) is not False:
            raise RuntimeError(f"Prohibited search enabled: {key}")
...
def fit_meta_allocator(
    ...
    if len(np.unique(y)) < 2:
        raise RuntimeError(
            "Meta allocator target contains one class"
        )
...
def outer_validation_observations(
    ...
    for timestamp, session in scored.groupby(
        ...
        primary_side = (
            exposure * PRIMARY_COST_BPS / 10_000.0
        )
        stress_side = (
            exposure * STRESS_COST_BPS / 10_000.0
        )
        primary = (
            (1.0 + gross)
            * (1.0 - primary_side) ** 2
...
def evaluate_gates(
    ...
    summary = {
        ...
        "median_mean_gross_exposure":
            float(metrics["mean_gross_exposure"].median()),
...
def run(
    ...
    manifest = {
        ...
        "phase": PHASE,
        "stage":
            "nested_walk_forward_tail_aware_four_model_ml_evaluation",
        "generated_at_utc":
