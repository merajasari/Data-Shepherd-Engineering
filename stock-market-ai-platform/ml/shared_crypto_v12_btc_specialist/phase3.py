"""Shared Crypto V12 BTC Specialist Phase 3.

Immutable predictive adjudication for V12.

Reads only Phase 2 predictive evidence and records whether the preregistered
BTC-specialist hierarchy qualified for portfolio simulation. If any of the nine
predictive gates failed, V12 is closed as rejected predictive evidence.

This phase performs no model fitting, class-weight changes, hyperparameter
changes, threshold tuning, probability calibration, portfolio simulation,
...
OUTPUT_ROOT = Path(
    "data/model/shared_crypto_v12_btc_specialist/phase3"
)
...
def adjudicate(
    ...
    if len(gate_detail) != 1:
        raise RuntimeError(
            "V12 Phase 2 must contain exactly one predictive gate row"
        )
    ...
    if missing:
        raise RuntimeError(
            f"V12 Phase 2 gate detail missing columns: {sorted(missing)}"
        )
    ...
    if total != 9:
        raise RuntimeError(
            "V12 expected exactly nine predictive gates"
        )
    if int(
        gate_result[
        ...
        raise RuntimeError(
            "V12 total predictive gate count is inconsistent"
        )
    if int(
        gate_result[
        ...
        raise RuntimeError(
            "V12 passed predictive gate count is inconsistent"
        )
    ...
    if bool(
        ...
        raise RuntimeError(
            "V12 all_predictive_gates_pass flag is inconsistent"
        )
    ...
    if str(
        ...
        raise RuntimeError(
            "V12 Phase 2 predictive status is inconsistent"
        )

    if len(summary) != 1:
        raise RuntimeError(
            "V12 Phase 2 metrics summary must contain exactly one row"
        )
    ...
    if missing:
        raise RuntimeError(
            f"V12 metrics summary missing columns: {sorted(missing)}"
        )

    if "predicted_sleeve" not in predictions.columns:
        raise RuntimeError(
            "V12 predictions are missing predicted_sleeve"
        )
    ...
    if sum(
        ...
        raise RuntimeError(
            "V12 predictions contain an unexpected sleeve class"
        )
    ...
    decision = {
        "status": (
            ...
            if all_pass
            else "REJECT_CURRENT_V12_PREDICTIVE_HYPOTHESIS"
        ),
...
def run(
    ...
    if (
        ...
        raise RuntimeError(
            "Unexpected V12 Phase 2 research version"
        )
    if (
        manifest.get(
        ...
        raise RuntimeError(
            "Unexpected V12 Phase 2 holdout boundary"
        )
    if (
        manifest.get(
            "stage1_model"
    ...
    if (
        ...
        raise RuntimeError(
            "V12 Phase 2 unexpectedly searched secondary models"
        )
    ...
    for key in (
        ...
        if (
            ...
            raise RuntimeError(
                f"V12 Phase 2 safety invariant failed: {key}"
            )
    ...
    payload = {
        ...
        **decision,
        "reason": (
            "V12 required all nine preregistered predictive gates to pass "
            "before portfolio simulation. The nonlinear BTC specialist passed "
            "seven gates, including both Stage 2 gates and the combined "
            "balanced-accuracy, macro-F1, MCC, and minimum-class-recall gates. "
            "It still failed Stage 1 BTC-versus-DEVIATE balanced accuracy and "
            "combined accuracy improvement versus the training-majority "
            "baseline. V12 is therefore rejected before portfolio simulation."
        ),
        "outputs": {
        ...
        "safety": {
            ...
            "v12_model_frozen": False,
            "paper_state_modified": False,
            "brokerage_orders": False,
            "automatic_promotion": False,
        },
        "next_step": (
            "Preserve V12 as rejected predictive evidence. Do not simulate "
            "the V12 portfolio, retune the Stage 1 booster, alter Stage 2, add "
            "thresholds, weaken gates, or score the future holdout. Any "
            "successor must be separately named and preregistered before fit."
        ),
    }
