"""StockEagle250 V2 Phase 2: fixed risk-overlay development evaluation.

Consumes the saved StockEagle250 V1 Ridge out-of-sample portfolio periods and
applies the single Phase-1-preregistered V10-style SPY regime exposure map.
There is no refit, candidate search, threshold search, exposure search, or use
of rows from the September 23 guard band or October 1 future sample.
"""
...
OUTPUT_ROOT = Path("data/model/stock_eagle_250_v2/phase2")
...
def validate_sources(
    ...
    if phase1_manifest.get("candidate_id") != V2_CANDIDATE:
    ...
    if phase1_manifest.get("v2_performance_calculated") is not False:
        problems.append("Phase 1 already reports V2 performance")
    ...
    if problems:
        raise RuntimeError("StockEagle250 V2 source validation failed:\n- " + "\n- ".join(problems))
    ...
    if (pd.to_numeric(ridge["selected_count"], errors="coerce") != POSITIONS_PER_COHORT).any():
    ...
    for column in ("gross_return", "net_return"):
        if not np.isfinite(pd.to_numeric(ridge[column], errors="coerce")).all():
...
def attach_market_state(
    ...
    states = market_state_frame(spy_returns)
    state_columns = [
        "timestamp_utc",
        "market_state",
        "gross_exposure",
    ...
    if merged["market_state"].isna().any():
        first = merged.loc[merged["market_state"].isna(), "timestamp_utc"].min()
        raise RuntimeError(f"Missing SPY market state for V1 decision {first}")
    if not merged["gross_exposure"].between(0.0, 1.0).all():
        raise RuntimeError("V2 market-state exposure is outside [0, 1]")
    return merged
...
def evaluate_folds(
    state_periods: pd.DataFrame,
    v1_metrics: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    metric_lookup = v1_metrics.set_index("fold_id")
    period_frames = []
    rows = []
    for fold_id, fold in state_periods.groupby("fold_id", sort=True):
        ...
        if not np.isclose(v1["net_return"], float(recorded["net_return"]), atol=1e-12):
        ...
        primary.insert(1, "evaluation", "v2_primary_10bps")
        stress.insert(1, "evaluation", "v2_stress_20bps")
        ...
        rows.append({
            ...
            "excess_vs_spy": p["net_return"] - float(recorded["spy_net_return"]),
...
def evaluate_gates(
    ...
    summary = {
        ...
        "drawdown_improvement_fold_fraction":
            float((fold_metrics["drawdown_improvement_vs_v1_ridge"] > 0.0).mean()),
        "median_fold_calmar": float(fold_metrics["primary_calmar"].median()),
        "median_v1_ridge_calmar": float(fold_metrics["v1_ridge_calmar"].median()),
    ...
    qualification = {
        "status": (
            ...
            if qualified
            else "REJECT_CURRENT_STOCK_EAGLE_250_V2_CANDIDATE"
        ),
...
def diagnostic_summaries(periods: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    primary = periods.loc[periods["evaluation"] == "v2_primary_10bps"].copy()
    regime = (
        primary.groupby("market_state", sort=True)
        .agg(
            periods=("net_return", "size"),
            mean_gross_exposure=("gross_exposure", "mean"),
            mean_unscaled_ridge_return=("gross_return", "mean"),
            mean_v2_net_return=("net_return", "mean"),
        )
        .reset_index()
    )
    primary["year"] = pd.to_datetime(primary["timestamp_utc"], utc=True).dt.year
    year = (
        ...
        .agg(
            ...
            mean_gross_exposure=("gross_exposure", "mean"),
            mean_v2_net_return=("net_return", "mean"),
            positive_period_fraction=("net_return", lambda x: float((x > 0.0).mean())),
        )
        .reset_index()
    )
    return regime, year
...
def run(
    ...
    state_periods = attach_market_state(ridge, spy_returns)
    period_results, fold_results = evaluate_folds(state_periods, v1_metrics)
    gates, summary, qualification = evaluate_gates(fold_results, contract)
    regime, year = diagnostic_summaries(period_results)

    output_root = Path(output_root)
    output_root.mkdir(parents=True, exist_ok=True)
    period_results.to_parquet(output_root / "period_results.parquet", index=False)
    fold_results.to_csv(output_root / "fold_metrics.csv", index=False)
    regime.to_csv(output_root / "regime_summary.csv", index=False)
    ...
    manifest = {
        ...
        "phase": PHASE,
        "stage": "fixed_v10_inspired_risk_overlay_development_evaluation",
        ...
        "outputs": {
            ...
            "regime_summary": str(output_root / "regime_summary.csv"),
...
def main(argv=None) -> None:
    ...
    parser.add_argument("--fold-metrics", type=Path, default=V1_FOLD_METRICS_PATH)
    parser.add_argument("--phase1-manifest", type=Path, default=PHASE1_MANIFEST_PATH)
    parser.add_argument("--spy-features", type=Path, default=SPY_FEATURE_PATH)
