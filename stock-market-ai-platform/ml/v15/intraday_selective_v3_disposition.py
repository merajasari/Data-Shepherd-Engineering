"""Identity and audit checks for the rejected V15 V3 development result."""
...
DISPOSITION_PATH = Path(__file__).with_name("intraday_selective_v3_disposition.json")
RESULT_PATH = Path(__file__).resolve().parents[2] / (
    "data/research/v15/intraday_selective_v3/latest_results.json"
)
EXPECTED_DISPOSITION_SHA256 = (
    "f8413758ce52bd4f536bf84072cd6fb2b404030335da11cafe855551899bc0f6"
)
...
def load_disposition(path: Path = DISPOSITION_PATH) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V15_V3_DISPOSITION_MUST_BE_OBJECT")
    unsigned = dict(payload)
    embedded = unsigned.pop("disposition_sha256", None)
    actual = canonical_sha256(unsigned)
    if embedded != actual or actual != EXPECTED_DISPOSITION_SHA256:
        raise ValueError("V15_V3_DISPOSITION_SHA_MISMATCH")
    if payload.get("decision") != "REJECTED_DO_NOT_ADVANCE_TO_PAPER_FORWARD":
        raise ValueError("V15_V3_DISPOSITION_DECISION_INVALID")
    diagnosis = payload.get("diagnosis", {})
    if diagnosis.get("one_trade_is_insufficient_evidence") is not True:
        raise ValueError("V15_V3_PARTICIPATION_DIAGNOSIS_MISSING")
    authority = payload.get("authority")
    if not isinstance(authority, Mapping):
        raise ValueError("V15_V3_DISPOSITION_AUTHORITY_INVALID")
    required_false = (
        "model_frozen",
...
def validate_result_artifact(
    ...
    if embedded != canonical_sha256(unsigned) or embedded != receipt["observed_result_sha256"]:
        raise ValueError("V15_V3_RESULT_SHA_MISMATCH")
    if result.get("contract_sha256") != receipt["contract_sha256"]:
        raise ValueError("V15_V3_RESULT_CONTRACT_MISMATCH")
    if result.get("source_manifest_sha256") != receipt["source_manifest_sha256"]:
        raise ValueError("V15_V3_RESULT_SOURCE_MISMATCH")
    ...
    for name in (
        ...
        if isinstance(expected, float):
            if not isinstance(actual, (int, float)) or abs(float(actual) - expected) > 1e-15:
                raise ValueError(f"V15_V3_RESULT_METRIC_MISMATCH:{name}")
        elif actual != expected:
            raise ValueError(f"V15_V3_RESULT_METRIC_MISMATCH:{name}")
    if result.get("gate_results") != receipt["gate_results"]:
        raise ValueError("V15_V3_RESULT_GATES_MISMATCH")
    return result
...
def main() -> None:
    ...
    print("V15 V3 DEVELOPMENT DISPOSITION")
    print("=" * 72)
    ...
    print("Observed trades: 1 / 181 sessions")
