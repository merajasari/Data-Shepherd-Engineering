"""V15 V5 gap-aware, risk-managed intraday development evaluation."""
...
CONTRACT_PATH = Path(__file__).with_name("intraday_risk_managed_v5_contract.json")
MANIFEST_PATH = (
...
def load_contract(path: Path = CONTRACT_PATH) -> dict[str, object]:
    ...
    if canonical_sha256(contract) != EXPECTED_CONTRACT_SHA256:
        failures.append("V15_V5_CONTRACT_SHA_MISMATCH")
    if contract.get("classification") != (
        "PREREGISTERED_POST_V4_DEVELOPMENT_CANDIDATE_NOT_FROZEN"
    ):
        failures.append("V15_V5_CLASSIFICATION_INVALID")
    ...
    if heritage.get("v4_outcomes_read_before_v5_design") is not True:
        failures.append("V15_V5_V4_DISCLOSURE_MISSING")
    if heritage.get("v4_results_reused_as_fresh_evidence") is not False:
        failures.append("V15_V5_V4_EVIDENCE_BOUNDARY_INVALID")
    if data.get("current_session_daily_close_prohibited") is not True:
        failures.append("V15_V5_DAILY_LEAKAGE_PROHIBITION_MISSING")
    if int(data.get("holding_bars", 0)) != 24:
        failures.append("V15_V5_HORIZON_INVALID")
    if int(data.get("entry_bar_index", -1)) != 6:
        failures.append("V15_V5_ENTRY_INDEX_INVALID")
    if int(data.get("scheduled_exit_bar_index", -1)) != 29:
        failures.append("V15_V5_EXIT_INDEX_INVALID")
    if int(data.get("minimum_eligible_intraday_sessions", 0)) < 300:
        failures.append("V15_V5_HISTORY_MINIMUM_TOO_SMALL")
    if model.get("type") != "NUMPY_RIDGE_ABSOLUTE_RETURN_REGRESSION":
        failures.append("V15_V5_MODEL_TYPE_INVALID")
    if int(model.get("feature_count", 0)) != len(HYBRID_FEATURE_NAMES):
        failures.append("V15_V5_FEATURE_COUNT_INVALID")
    if float(risk.get("protective_stop_loss_fraction", math.nan)) != 0.02:
        failures.append("V15_V5_STOP_INVALID")
    if risk.get("gap_fill") != "BAR_OPEN_WHEN_BAR_OPEN_IS_BELOW_STOP_PRICE":
        failures.append("V15_V5_GAP_FILL_INVALID")
    if risk.get("stop_parameter_grid_search") is not False:
        failures.append("V15_V5_STOP_TUNING_PROHIBITION_MISSING")
    if risk.get("stop_applies_to_v15_and_MATCHED_STOCK_CONTROLS") is not True:
        failures.append("V15_V5_STOP_CONTROL_PARITY_INVALID")
    if tuple(nested.get("candidate_top_n", ())) != (1, 3, 5):
        failures.append("V15_V5_TOPK_GRID_INVALID")
    if tuple(nested.get("candidate_participation_quantiles", ())) != (
        0.5,
        0.65,
        0.8,
    ):
        failures.append("V15_V5_QUANTILE_GRID_INVALID")
    if int(nested.get("validation_subwindow_count", 0)) != 2:
        failures.append("V15_V5_SUBWINDOW_COUNT_INVALID")
    if nested.get("selection_data") != "OUTER_TRAINING_SESSIONS_ONLY":
        failures.append("V15_V5_NESTED_SELECTION_INVALID")
    if nested.get("outer_test_outcomes_used_for_selection") is not False:
        failures.append("V15_V5_OUTER_TEST_SELECTION_PROHIBITION_MISSING")
    if portfolio.get("cost_applied_only_when_trade_occurs") is not True:
        failures.append("V15_V5_COST_APPLICATION_INVALID")
    if float(portfolio.get("no_trade_return", math.nan)) != 0.0:
        failures.append("V15_V5_CASH_RETURN_INVALID")
    if evaluation.get("development_evidence_only") is not True:
        failures.append("V15_V5_DEVELOPMENT_BOUNDARY_MISSING")
    if evaluation.get("fresh_paper_boundary_required_after_development") is not True:
        failures.append("V15_V5_FRESH_BOUNDARY_MISSING")
    required_false = (
        "model_frozen",
    ...
    if any(authority.get(name) is not False for name in required_false):
        failures.append("V15_V5_AUTHORITY_INVALID")
    if authority.get("development_only") is not True:
        failures.append("V15_V5_DEVELOPMENT_AUTHORITY_INVALID")
    if authority.get("human_review_required") is not True:
        failures.append("V15_V5_REVIEW_BOUNDARY_INVALID")
    if failures:
...
def _gap_aware_stop_return(
    ...
    *,
    ...
    if not math.isfinite(entry) or entry <= 0.0:
        raise ValueError("V15_V5_ENTRY_PRICE_INVALID")
    ...
    for bar_index in range(entry_bar_index, exit_bar_index + 1):
        ...
        if float(bar["low"]) <= stop_price:
            ...
            if not math.isfinite(fill) or fill <= 0.0:
                raise ValueError("V15_V5_STOP_FILL_INVALID")
...
def _score_configuration(
    ...
    *,
    ...
    if len(sessions) % subwindow_count != 0:
        raise ValueError("V15_V5_VALIDATION_SUBWINDOWS_UNEVEN")
...
def select_configuration(
    ...
    if inner_training_stop < int(nested["minimum_inner_training_sessions"]):
        raise ValueError("V15_V5_INNER_TRAINING_HISTORY_INSUFFICIENT")
    ...
    if len(training_rows) < int(contract["model"]["minimum_training_rows"]):
        raise ValueError(f"V15_V5_TRAINING_ROWS_BELOW_MINIMUM:{len(training_rows)}")
...
def evaluate_walk_forward(
    ...
    *,
    ...
    if len(sessions) < minimum_history:
        raise ValueError(
            f"V15_V5_INSUFFICIENT_ELIGIBLE_SESSIONS:{len(sessions)}<{minimum_history}"
        )
    ...
    if available < int(evaluation["minimum_final_test_sessions"]):
        raise ValueError("V15_V5_INSUFFICIENT_TEST_SESSIONS")
    ...
    result: dict[str, object] = {
        ...
        "status": (
            "V15_V5_DEVELOPMENT_GATES_PASSED_NOT_FROZEN"
            if passed
            else "V15_V5_REJECTED_DEVELOPMENT_CANDIDATE"
        ),
...
def run(
    *,
    ...
    validate_v4_result_artifact()
    if contract["heritage"]["v4_disposition_sha256"] != disposition[
        "disposition_sha256"
    ]:
        raise ValueError("V15_V5_V4_DISPOSITION_BINDING_MISMATCH")
    v4_contract = load_v4_contract()
    if canonical_sha256(v4_contract) != contract["heritage"][
        "v4_feature_contract_sha256"
    ]:
        raise ValueError("V15_V5_V4_FEATURE_CONTRACT_MISMATCH")
    if progress:
    ...
    if sorted(symbols) != sorted(symbol for symbol in intraday if symbol != "SPY"):
        raise ValueError("V15_V5_V14_UNIVERSE_MISMATCH")
    ...
    result["v4_disposition_sha256"] = disposition["disposition_sha256"]
...
def main() -> None:
    print("V15 V5 RISK-MANAGED 120-MINUTE INTRADAY DEVELOPMENT")
    print("=" * 80)
    try:
