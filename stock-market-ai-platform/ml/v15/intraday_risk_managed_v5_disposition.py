"""Identity and audit checks for the rejected V15 V5 development result."""
...
DISPOSITION_PATH = Path(__file__).with_name(
    "intraday_risk_managed_v5_disposition.json"
)
RESULT_PATH = Path(__file__).resolve().parents[2] / (
    "data/research/v15/intraday_risk_managed_v5/latest_results.json"
)
EXPECTED_DISPOSITION_SHA256 = (
    "21d9a0cc1ab9efc3cd68ff442b0252ae5dd6a77854184ce9804d748ae628e67f"
)
...
def load_disposition(path: Path = DISPOSITION_PATH) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V15_V5_DISPOSITION_MUST_BE_OBJECT")
    unsigned = dict(payload)
    embedded = unsigned.pop("disposition_sha256", None)
    actual = canonical_sha256(unsigned)
    if embedded != actual or actual != EXPECTED_DISPOSITION_SHA256:
        raise ValueError("V15_V5_DISPOSITION_SHA_MISMATCH")
    if payload.get("decision") != "REJECTED_DO_NOT_ADVANCE_TO_PAPER_FORWARD":
        raise ValueError("V15_V5_DISPOSITION_DECISION_INVALID")
    diagnosis = payload.get("diagnosis", {})
    if diagnosis.get("stability_first_nested_selection_required_for_v6") is not True:
        raise ValueError("V15_V5_STABILITY_DIAGNOSIS_MISSING")
    if diagnosis.get("development_gate_weakening_prohibited") is not True:
        raise ValueError("V15_V5_GATE_WEAKENING_PROHIBITION_MISSING")
    authority = payload.get("authority")
    if not isinstance(authority, Mapping):
        raise ValueError("V15_V5_DISPOSITION_AUTHORITY_INVALID")
    required_false = (
        "model_frozen",
...
def validate_result_artifact(
    ...
    if embedded != canonical_sha256(unsigned) or embedded != receipt["observed_result_sha256"]:
        raise ValueError("V15_V5_RESULT_SHA_MISMATCH")
    if result.get("contract_sha256") != receipt["contract_sha256"]:
        raise ValueError("V15_V5_RESULT_CONTRACT_MISMATCH")
    if result.get("source_manifest_sha256") != receipt["source_manifest_sha256"]:
        raise ValueError("V15_V5_RESULT_SOURCE_MISMATCH")
    ...
    for name in (
        ...
        if result.get(name) != evidence[name]:
            raise ValueError(f"V15_V5_RESULT_METRIC_MISMATCH:{name}")
    if result.get("gate_results") != receipt["gate_results"]:
        raise ValueError("V15_V5_RESULT_GATES_MISMATCH")
    return result
...
def main() -> None:
    ...
    print("V15 V5 DEVELOPMENT DISPOSITION")
    print("=" * 72)
    ...
    print("Observed result: +12.37% net | 55 trades | 9.75% drawdown")
