"""Shared Crypto V5 Selective Rank Phase 4: immutable adjudication.
...
PHASE3_ROOT = Path("data/model/shared_crypto_v5_selective_rank/phase3")
OUTPUT_ROOT = Path("data/model/shared_crypto_v5_selective_rank/phase4")
...
def adjudicate(gates: pd.DataFrame, summary: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    if len(gates) != 1:
        raise RuntimeError("V5 Phase 3 must contain exactly one preregistered policy result")
    ...
    decision = {
        "status": (
            ...
            if qualified else "REJECT_CURRENT_V5_POLICY_FAMILY"
        ),
...
def run(phase3_root: Path = PHASE3_ROOT, output_root: Path = OUTPUT_ROOT) -> dict:
    ...
    if int(manifest.get("candidate_count", 0)) != 1:
        raise RuntimeError("V5 Phase 3 did not evaluate exactly one policy")
    ...
    payload = {
        ...
        **decision,
        "reason": (
            "The single preregistered V5 policy may advance only if every gate "
            "passes. Relative outperformance of a losing benchmark is not "
            "sufficient when absolute-return, BTC-relative, drawdown, stability, "
            "profit-concentration, and stress-cost gates fail."
        ),
        "outputs": {
        ...
        "safety": {
            ...
            "v5_model_frozen": False,
            "paper_state_modified": False,
            "brokerage_orders": False,
            "automatic_promotion": False,
        },
        "next_step": (
            "Preserve V5 as rejected development evidence. Any successor must "
            "be a separately named and preregistered hypothesis; do not tune V5 "
