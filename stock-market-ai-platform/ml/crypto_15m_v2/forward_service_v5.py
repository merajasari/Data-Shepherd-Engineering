"""Shared Crypto V2 Clean Forward V5 isolated paper-evaluation entrypoint.

V5 reuses the unchanged frozen Phase 5 HGB artifact and confirm-2 policy while
placing every stateful output in a new lane. Clean Forward V3 is preserved as
an immutable interrupted parent and is never resumed or backfilled. The V4
contract boundary passed before installation and is preserved in source as a
missed, zero-evidence lane.

Preregistered boundary: 2026-09-24 07:00 UTC (midnight America/Los_Angeles).
...
CLEAN_ROOT = PHASE5_ROOT / "clean_forward_v5"
CLEAN_START = pd.Timestamp("2026-09-24T07:00:00Z")
MISSED_V4_START = pd.Timestamp("2026-09-23T07:00:00Z")
CLEAN_LANE_ID = "shared_crypto_v2_clean_forward_v5"
CLEAN_LANE_NAME = "Shared Crypto V2 Clean Forward V5"


def configure() -> None:
    """Point the proven service implementation at the isolated V5 contract."""
    ...
    # V3 is the last lane with evidence and remains an immutable parent.
