...
The comparison intentionally excludes V6 and V7.  It contains:
  * V4 causal walk-forward reconstruction
  * V5 frozen development portfolio diagnostic (10 bps)
  * V8 exact frozen DISTANCE_ONLY Top-10 / 5-session / next-open / 10-bps policy
  * SPY buy-and-hold benchmark

Every strategy is independently normalized to the same hypothetical $100,000
starting capital at its own first scientifically eligible observation.  No live
paper-account balance is appended to any reconstructed curve.

This script is research/display only.  It does not mutate paper state, frozen
models, holdout journals, or brokerage settings.
"""
...
def _series_record(model_id: str, label: str, rows: list[dict], methodology: str, status: str) -> dict:
    if not rows:
    ...
    rows = [
        {
            ...
            "equity": float(r["equity"]) * scale,
            "source": "V4 causal walk-forward reconstruction",
        }
        for r in raw
        if r.get("timestamp") is not None and r.get("equity") is not None
    ]
...
def _load_v5() -> dict:
    ...
    t["net_return"] = pd.to_numeric(t["gross_return_5d"], errors="coerce") - pd.to_numeric(
        t["turnover"], errors="coerce"
    ) * V5_COST_BPS / 10000.0
    t = t[np.isfinite(t["net_return"])].copy()
    equity = STARTING_CAPITAL
    rows = [{"timestamp": _iso(t.iloc[0]["timestamp_utc"]), "equity": equity, "source": "V5 starting capital"}]
...
def _load_v8() -> dict:
    ...
    required = {
        "score_id", "cohort_offset", "entry_timestamp_utc", "exit_timestamp_utc",
        "cost_bps_per_dollar_traded", "net_portfolio_return",
    }
    missing = sorted(required - set(p.columns))
    if missing:
        raise ValueError("V8 economic-period file missing: " + ", ".join(missing))
    p = p[
        (p["score_id"] == V8_SCORE_ID)
        & (pd.to_numeric(p["cost_bps_per_dollar_traded"], errors="coerce") == V8_COST_BPS)
    ].copy()
    ...
    # The frozen strategy retains all five staggered cohort offsets.  Represent
    # that contract as five equal 20% sleeves and aggregate them into one account.
    cohort_equity = {int(offset): STARTING_CAPITAL / 5.0 for offset in range(5)}
    ...
    for exit_ts, g in p.groupby("exit_timestamp_utc", sort=True):
        for row in g.itertuples(index=False):
            ...
            if offset not in cohort_equity:
                continue
...
def _load_spy(start_ts: pd.Timestamp) -> dict:
    ...
    df = df[(df["timestamp_utc"] >= start_ts) & np.isfinite(df["close"]) & (df["close"] > 0)].copy()
    ...
    rows = [
        {
            "timestamp": _iso(r.timestamp_utc),
            "equity": float(STARTING_CAPITAL * float(r.close) / base),
            "source": "SPY buy-and-hold benchmark",
        }
    ...
    return _series_record(
        ...
        "SPY",
        rows,
        "SPY buy-and-hold benchmark normalized to the same $100,000 starting capital at the earliest V4 comparison date.",
        "benchmark",
    )
...
def main() -> None:
    ...
    payload = {
        ...
        "comparison_policy": (
            ...
            "Live paper-account balances are intentionally excluded from these lines. Model curves begin only when "
            "their own scientifically eligible historical evidence begins; no history is backfilled before eligibility."
        ),
        "holdout_note": (
            "V8 historical reconstruction is development-era evidence only. The genuine 2026-09-01+ forward holdout "
            "remains a separate append-only evidence stream and is not retroactively represented as holdout performance."
        ),
        "series": series,
        "research_safety": {
    ...
    for s in series:
        print(
            f"{s['model_id']:>3}: {s['start_timestamp']} -> {s['end_timestamp']} | "
            f"obs={s['observations']:,} | ${s['ending_equity']:,.2f} | {s['total_return_pct']:+.2f}%"
        )
