...
def _hourly_benchmarks(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(h) != len(econ_ts):
        raise RuntimeError(
            f"Benchmark timestamp alignment lost rows: expected {len(econ_ts)}, got {len(h)}"
        )

    out = []
    for state, col in [
        ("XRP", "forward_return_1h"),
        ("BTC", "btc_forward_return_1h"),
        ("CASH", None),
    ]:
        r = (
            np.zeros(len(h), dtype=float)
            if col is None
            else pd.to_numeric(h[col], errors="coerce").fillna(0.0).to_numpy(float)
        )
        eq = np.cumprod(1.0 + r)
...
def main():
    ...
    fold_stability = (
        ...
        .agg(
            ...
            median_fold_equity=("ending_equity", "median"),
            worst_fold_drawdown=("max_drawdown", "min"),
        )
        .reset_index()
    )

    best_5 = summary[summary["cost_bps"].eq(5)].sort_values(
        "ending_equity", ascending=False
    ).head(1)
    ...
    if candidate is None:
        ...
        and not bool(
            cost_path.loc[
                cost_path["policy_id"].eq(candidate),
                "cost_hurdle_changes_state_path",
            ].iloc[0]
        )
    ):
