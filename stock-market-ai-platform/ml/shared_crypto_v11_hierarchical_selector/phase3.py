"""Shared Crypto V11 Hierarchical Selector Phase 3.

Immutable predictive adjudication for V11.

Reads only Phase 2 predictive evidence and records whether the preregistered
two-stage hierarchy qualified for portfolio simulation. If any of the nine
predictive gates failed, V11 is closed as rejected predictive evidence.

This phase performs no model fitting, class-weight changes, threshold tuning,
probability calibration, portfolio simulation, future-holdout scoring, model
freezing, paper activation, or brokerage action.
"""
...
OUTPUT_ROOT = Path(
    "data/model/shared_crypto_v11_hierarchical_selector/phase3"
)
...
def adjudicate(
    ...
    if len(gate_detail) != 1:
        raise RuntimeError(
            "V11 Phase 2 must contain exactly one predictive gate row"
        )
    ...
    if missing:
        raise RuntimeError(
            f"V11 Phase 2 gate detail missing columns: {sorted(missing)}"
        )
    ...
    if total != 9:
        raise RuntimeError("V11 expected exactly nine predictive gates")
    if int(gate_result["total_predictive_gate_count"]) != total:
        raise RuntimeError(
            "V11 total predictive gate count is inconsistent"
        )
    if int(gate_result["passed_predictive_gate_count"]) != passed:
        raise RuntimeError(
            "V11 passed predictive gate count is inconsistent"
        )

    all_pass = bool(passed == total)
    if bool(gate_result["all_predictive_gates_pass"]) != all_pass:
        raise RuntimeError(
            "V11 all_predictive_gates_pass flag is inconsistent"
        )
    ...
    if str(gate_result["status"]) != expected_status:
        raise RuntimeError(
            "V11 Phase 2 predictive status is inconsistent"
        )

    if len(summary) != 1:
        raise RuntimeError(
            "V11 Phase 2 metrics summary must contain exactly one row"
        )
    ...
    if missing:
        raise RuntimeError(
            f"V11 metrics summary missing columns: {sorted(missing)}"
        )

    if "predicted_sleeve" not in predictions.columns:
        raise RuntimeError(
            "V11 predictions are missing predicted_sleeve"
        )
    ...
    if sum(counts.values()) != len(predictions):
        raise RuntimeError(
            "V11 predictions contain an unexpected sleeve class"
        )
    ...
    disposition["failed_gates"] = ",".join(failed_gates)

    decision = {
        "status": (
            "QUALIFIED_FOR_POLICY_SIMULATION"
            if all_pass
            else "REJECT_CURRENT_V11_PREDICTIVE_HYPOTHESIS"
        ),
        ...
        "observed_predictive_metrics": {
            "median_stage1_balanced_accuracy": float(
                summary_row["median_stage1_balanced_accuracy"]
            ),
            "median_stage1_mcc": float(
                summary_row["median_stage1_mcc"]
            ),
            "median_stage2_balanced_accuracy": float(
                summary_row["median_stage2_balanced_accuracy"]
            ),
            "median_stage2_mcc": float(
                summary_row["median_stage2_mcc"]
            ),
            "median_combined_balanced_accuracy": float(
                summary_row["median_combined_balanced_accuracy"]
            ),
            "median_combined_macro_f1": float(
                summary_row["median_combined_macro_f1"]
            ),
            ...
            "median_combined_multiclass_mcc": float(
                summary_row["median_combined_multiclass_mcc"]
            ),
            "median_combined_minimum_class_recall": float(
                summary_row["median_combined_minimum_class_recall"]
            ),
            "median_combined_btc_recall": float(
                summary_row["median_combined_btc_recall"]
            ),
            "median_combined_alt_recall": float(
                summary_row["median_combined_alt_recall"]
            ),
            "median_combined_cash_recall": float(
                summary_row["median_combined_cash_recall"]
            ),
        },
        "observed_prediction_counts": counts,
        "observed_prediction_rows": int(len(predictions)),
    }
...
def run(
    ...
    if manifest.get("research_version") != RESEARCH_VERSION:
        raise RuntimeError(
            "Unexpected V11 Phase 2 research version"
        )
    if manifest.get("future_holdout_start_utc") != HOLDOUT:
        raise RuntimeError(
            "Unexpected V11 Phase 2 holdout boundary"
        )
    if manifest.get("primary_model") != "linear_svc":
        raise RuntimeError(
            "Unexpected V11 Phase 2 primary model"
        )
    if float(manifest.get("linear_svc_C")) != 0.25:
        raise RuntimeError(
            "Unexpected V11 Phase 2 LinearSVC C"
        )
    if manifest.get("class_weight") != "balanced":
        raise RuntimeError(
            "Unexpected V11 Phase 2 class weighting"
        )
    if int(manifest.get("secondary_model_count", -1)) != 0:
        raise RuntimeError(
            "V11 Phase 2 unexpectedly searched secondary models"
        )
    ...
    for key in (
        ...
        if safety.get(key) is not False:
            raise RuntimeError(
                f"V11 Phase 2 safety invariant failed: {key}"
            )
    ...
    gate_result = json.loads(
        (phase2_root / "predictive_gate_result.json").read_text(
            encoding="utf-8"
        )
    )
    summary = pd.read_csv(
    ...
    predictions = pd.read_parquet(
        phase2_root / "daily_predictions.parquet"
    )
    ...
    payload = {
        ...
        **decision,
        "reason": (
            "V11 required all nine preregistered predictive gates to pass "
            "before portfolio simulation. Stage 2 ALT-versus-CASH passed both "
            "of its gates, but Stage 1 BTC-versus-DEVIATE failed its balanced-"
            "accuracy gate. The combined hierarchy also failed balanced "
            "accuracy, macro F1, and accuracy improvement versus the training-"
            "majority baseline. V11 is therefore rejected before portfolio "
            "simulation."
        ),
        "outputs": {
        ...
        "safety": {
            ...
            "class_weight_changed_after_results": False,
            "linear_svc_C_changed_after_results": False,
        ...
        "next_step": (
            "Preserve V11 as rejected predictive evidence. Any successor must "
            "be separately named and preregistered before fitting. The observed "
            "weak link is BTC-versus-DEVIATE; do not retune V11 after results, "
            "simulate its portfolio, weaken gates, or score the future holdout."
        ),
    }
...
def main(argv=None) -> None:
    ...
    print(json.dumps(
        run(args.phase2_root, args.output_root),
        indent=2,
    ))
