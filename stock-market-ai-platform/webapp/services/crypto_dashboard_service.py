...
shadow/forward service state. Presentation only: never fits models or orders.
"""
...
def get_crypto_dashboard_payload():
    primary=_primary_daily(); btc=_btc_daily(); ranking_timestamp,rankings=_latest_rankings(); live=_v2_live()
    if primary.empty:return {"available":False,"message":"Crypto V1 Phase 4 artifacts are not available on this machine.","universe":list(CRYPTO_UNIVERSE),"rankings":rankings,"ranking_timestamp":ranking_timestamp,"live_v2":live}
