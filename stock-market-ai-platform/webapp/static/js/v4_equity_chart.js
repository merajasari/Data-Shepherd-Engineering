(() => {
  ...
  function renderHoldoutInteraction(d){
    ...
    const hsvg=card.querySelector('#smc-holdout-chart'), empty=card.querySelector('#smc-holdout-empty'), htip=card.querySelector('#smc-holdout-tooltip'), hwrap=hsvg?.closest('.smc-holdout-chart-wrap');
    ...
    card.querySelectorAll('[data-holdout-event]').forEach(button=>{const visible=holdoutActive.has(button.dataset.holdoutEvent);button.classList.toggle('off',!visible);button.setAttribute('aria-pressed',String(visible));});
