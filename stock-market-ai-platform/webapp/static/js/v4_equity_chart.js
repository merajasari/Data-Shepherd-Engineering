(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:1050px){.smc-workspace{grid-template-columns:1fr}.smc-sidebar .smc-metrics{grid-template-columns:repeat(4,1fr)}}@media(max-width:650px){.smc-sidebar .smc-metrics{grid-template-columns:1fr 1fr}}
  ...
  function buildModelToggles(){
    ...
    ORDER.forEach(id=>{
      ...
      const meanings={
        V4:'Earlier cross-sectional ranking-model reconstruction.',
        V5:'Later model generation; valid evidence begins in 2021.',
        V8:'Frozen model contract. Its strategy and execution assumptions are locked.',
        V10:'Reconstructed development-period result. It is not live trading performance.',
        SPY:'Passive S&P 500 benchmark on the same hypothetical starting-capital basis.'
      };
      const row=document.createElement('div');
      row.className='smc-model-row';
      const b=document.createElement('button');
      b.type='button';b.className='smc-model';b.dataset.model=id;
      b.setAttribute('aria-pressed','true');
      b.title='Click to show or hide this line';
      b.innerHTML=`<span class="smc-dot" style="background:${COLORS[id]}"></span><strong title="${meanings[id]}">${s.label}</strong><span class="smc-model-value">${money(s.rows.at(-1)?.equity)} · ${pct(s.totalReturnPct)}</span><span class="smc-model-start" title="The first date this model has scientifically eligible evidence; earlier dates are intentionally not backfilled.">Eligible since ${date(s.rows[0]?.t)}</span><span class="smc-model-state">SHOWN</span>`;
      const only=document.createElement('button');
      only.type='button';only.className='smc-only';only.dataset.modelOnly=id;
      only.textContent='ONLY';only.title=`Show only ${s.label}`;
      row.append(b,only);
  ...
  function render(){
    ...
    set('smc-visible-range',`${date(a)} → ${date(b)}`);set('smc-model-count',String(active.size));set('smc-latest-date',date(endTime()));set('smc-zoom-status',zoomLevel>1?`${zoomLevel.toFixed(1)}× ZOOM`:'FULL RANGE');const availableCount=ORDER.filter(id=>series[id]).length;set('smc-range-message',active.size===availableCount?`${range} selected — all ${availableCount} models shown.`:`${range} selected — ${active.size} of ${availableCount} models shown.`);
