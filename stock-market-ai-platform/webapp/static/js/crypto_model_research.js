(() => {
  ...
  async function init(){
    ...
    try{const r=await fetch('/api/crypto-model-comparison',{credentials:'same-origin'});payload=await r.json();if(!r.ok||!payload.available)throw new Error(payload.error||`HTTP ${r.status}`);enabled=new Set(payload.series.map(s=>s.model_id));status.textContent=`${payload.series.length} ELIGIBLE SERIES · GENERATED ${(payload.generated_at_utc||'').slice(0,10)}`;document.getElementById('cmr-policy').textContent=payload.common_clock_policy;const unavailable=document.getElementById('cmr-unavailable');if(payload.unavailable_series?.length){unavailable.hidden=false;unavailable.innerHTML='<strong>Not charted:</strong> '+payload.unavailable_series.map(s=>`${esc(s.label)} — ${esc(s.reason)}`).join(' · ')}render()}catch(e){status.textContent='COMPARISON NOT BUILT';document.getElementById('cmr-error').hidden=false;document.getElementById('cmr-error').textContent=`${e.message} Run: python -m ml.build_crypto_model_comparison`}}
  document.querySelectorAll('[data-range]').forEach(b=>b.onclick=()=>{range=b.dataset.range;render()});init();
})();
