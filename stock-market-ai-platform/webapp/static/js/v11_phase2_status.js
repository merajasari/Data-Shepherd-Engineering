(() => {
  ...
  const renderAttempt = (attempt, archived = false) => {
    ...
    if (!attempt?.status) {
      ...
      set('attempt-summary', archived
        ? 'No collection attempt was preserved before V11 entered read-only archival status.'
        : 'No persisted post-patch collection attempt yet. The next natural scheduled session will populate this panel.');
      set('attempt-time', '—');
      set('attempt-window', '—');
      ...
      if (reasonsHost) {
        ...
        item.textContent = archived
          ? 'Collection is disabled. V11 is retained only as scientific lineage and locked V13 support code.'
          : 'Waiting for the next natural scheduled collection. No manual evidence run is required.';
  ...
  const refresh = async () => {
    try {
      ...
      if (badge) badge.className = `v11p-badge ${displayStatus === 'FRESH_EVIDENCE_ACTIVE' || archived ? 'v11p-good' : displayStatus === 'ALERT' ? 'v11p-alert' : 'v11p-wait'}`;
      ...
      set('requests', archived
        ? '0 (ARCHIVED)'
        : `${data.maximum_tiingo_requests_per_session ?? 404}/${data.tiingo_hourly_request_limit ?? 500}`);
      set('orders', data.brokerage_orders === false ? 'OFF' : 'ALERT');
      set('latest-session', data.latest_session_date || (archived ? 'None collected before archival' : 'Awaiting first fresh session'));
      ...
      set('windows', archived
        ? 'ARCHIVED — NO COLLECTION WINDOWS'
        : (data.collection_windows_eastern || []).join(' · '));
      set('catch-up', data.catch_up_enabled ? data.catch_up_policy : (data.catch_up_policy || 'DISABLED'));
      ...
      renderAttempt(lastAttempt, archived);
      ...
      const detail = archived
        ? `V11 collection and alerts are disabled. ${data.journal_event_count ?? 0} journal event${data.journal_event_count === 1 ? '' : 's'} and all research artifacts are preserved; locked V11 code required by V13 remains in place.`
        : failures.length
        ? failures.join(' · ')
      ...
      set('detail', `${detail} Paper only; no live orders or V8/V10/V13 evidence changes.`);
    } catch (error) {
