(()=>{
  ...
  let data=new Map(),todayData=new Map(),todayLoading=false,todayLoadedAt=0,quotes={};
  ...
  function marketOpenMs(nowMs=Date.now()){const n=easternParts(nowMs);let guess=Date.UTC(n.year,n.month-1,n.day,9,30,0);const gp=easternParts(guess);const represented=Date.UTC(gp.year,gp.month-1,gp.day,gp.hour,gp.minute,gp.second);return Date.UTC(n.year,n.month-1,n.day,9,30,0)-(represented-guess);}
  function historicalCut(rows){const end=Date.now();let out=rows.filter(x=>x.t>=end-days[range]*864e5&&x.t<=end);out=appendLiveCurrentRows(out);if(zoom>1&&out.length>2){const n=Math.max(2,Math.floor(out.length/zoom)),max=out.length-n,start=Math.round(max*pan);out=out.slice(start,start+n);}return out;}
  function appendLiveCurrentRows(rows){return rows;}
  function appendLive(symbol,rows){const out=[...rows],live=Number(quotes[symbol]?.reference_price);if(out.length&&Number.isFinite(live)){const liveTs=Date.parse(quotes[symbol]?.timestamp||'')||Date.now(),last=out.at(-1);if(!last||Math.abs(last.t-liveTs)>500)out.push({t:liveTs,price:live,live:true});else out[out.length-1]={t:liveTs,price:live,live:true};}return out;}
  function selectedRangeRows(symbol,rows){const now=Date.now();if(range==='TODAY'){const source=todayData.get(symbol)||[];return appendLive(symbol,[...source].filter(x=>x.t>=marketOpenMs(now)&&x.t<=now));}return appendLive(symbol,rows.filter(x=>x.t>=now-days[range]*864e5&&x.t<=now));}
  ...
  function renderRankingList(rank){
    rankRange.textContent=range==='TODAY'?'TODAY':range;
    if(!rank.length){rankList.innerHTML=`<div class="muted">${range==='TODAY'&&todayLoading?'Loading market session…':'No ranked stocks available.'}</div>`;return;}
    const maxMagnitude=Math.max(...rank.map(x=>Math.abs(x.totalRet)),0.0001);
    rankList.innerHTML=rank.map((x,i)=>{
      const pct=x.totalRet*100;
      const width=Math.max(7,Math.min(100,Math.abs(x.totalRet)/maxMagnitude*100));
      const company=names[x.symbol]||x.symbol;
      return `<div class="tlrank-row ${i<3?'top3':''}" data-rank-symbol="${x.symbol}"><div class="tlrank-badge">#${i+1}</div><div><div class="tlrank-symbol">${x.symbol}</div><div class="tlrank-company" title="${company}">${company}</div><div class="tlrank-track"><div class="tlrank-fill" style="width:${width.toFixed(1)}%"></div></div></div><div><div class="tlrank-caption">${range==='TODAY'?'TODAY':'TOTAL'} CHANGE</div><div class="tlrank-return ${pct>=0?'pos':'neg'}">${pct>=0?'+':''}${pct.toFixed(2)}%</div></div></div>`;
    }).join('');
  }

  function render(){
    svg.innerHTML='';tip.style.display='none';
    const rank=ranking();renderRankingList(rank);card.querySelectorAll('[data-r]').forEach(b=>b.classList.toggle('on',b.dataset.r===range));card.querySelectorAll('[data-m]').forEach(b=>b.classList.toggle('on',b.dataset.m===mode));
    ...
    const shown=rank.filter(x=>!hidden.has(x.symbol));if(!shown.length){const t=E('text',{x:500,y:195,'text-anchor':'middle',fill:'#91a6c2'});t.textContent=range==='TODAY'?(todayLoading?'Loading market session…':'No intraday bars are available since market open.'):'Loading history…';geo=null;stamp.textContent=range==='TODAY'?(todayLoading?'TODAY · LOADING':'TODAY'):'LOADING';return;}
    ...
    if(range!=='TODAY'){
      ...
      if(aligned.every(x=>x.rows.length>=2))plotted=aligned;
      else commonStart=null;
    }

    const W=1000,H=390,p={l:66,r:25,t:20,b:40};let vals=plotted.flatMap(x=>x.rows.map(q=>mode==='growth'?q.price/x.rows[0].price-1:q.price));let mn=Math.min(...vals),mx=Math.max(...vals);if(mode==='growth'){mn=Math.min(0,mn);mx=Math.max(0,mx);}const sp=Math.max(mx-mn,mode==='growth'?.002:1);mn-=sp*.1;mx+=sp*.1;const ts=plotted.flatMap(x=>x.rows.map(q=>q.t)),now=Date.now();let t0,t1;if(range==='TODAY'){t0=marketOpenMs(now);t1=now;}else if(zoom===1){t1=now;t0=commonStart??Math.min(...ts);}else{t0=commonStart??Math.min(...ts);t1=Math.max(now,Math.max(...ts));}const td=Math.max(1,t1-t0),X=t=>p.l+(W-p.l-p.r)*(t-t0)/td,Y=v=>p.t+(H-p.t-p.b)*(1-(v-mn)/(mx-mn));
    ...
    [t0,t0+td/2,t1].forEach((t,i)=>{const label=E('text',{x:X(t),y:H-15,'text-anchor':i===0?'start':i===2?'end':'middle',fill:'#91a6c2','font-size':'11'});label.textContent=range==='TODAY'?new Date(t).toLocaleString(undefined,{hour:'numeric',minute:'2-digit'}):new Date(t).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'2-digit'});});
    E('line',{id:'cross',y1:p.t,y2:H-p.b,stroke:'#f2f6ff','stroke-dasharray':'5 4',visibility:'hidden'});E('circle',{id:'dot',r:5.5,fill:'#fff',stroke:'#07101f','stroke-width':2,visibility:'hidden'});geo={W,H,p,t0,td,X,Y,shown:plotted,rank};stamp.textContent=`${range==='TODAY'?'TODAY':range} · LIVE`;card.querySelector('#tlz').textContent=zoom===1?'FULL RANGE':`${zoom.toFixed(1)}× ZOOM`;note.textContent=range==='TODAY'?'TODAY = current U.S. market session from the 9:30 AM ET open through the latest live IEX mark.':'Historical view starts at the first timestamp where all displayed lines have data; latest live IEX marks are appended to the series.';
  }

  svg.addEventListener('pointermove',e=>{
    if(!geo)return;const {W,H,p,t0,td,X,Y,shown,rank}=geo,rect=svg.getBoundingClientRect(),sx=(e.clientX-rect.left)*W/rect.width,sy=(e.clientY-rect.top)*H/rect.height;if(sx<p.l||sx>W-p.r)return;const target=t0+(sx-p.l)/(W-p.l-p.r)*td;let best;shown.forEach(x=>{const q=x.rows.reduce((a,b)=>Math.abs(b.t-target)<Math.abs(a.t-target)?b:a),v=mode==='growth'?q.price/x.rows[0].price-1:q.price,y=Y(v),d=Math.abs(y-sy);if(!best||d<best.d)best={x,q,v,y,d};});
    if(!best||best.d>30){hoverActive=false;tip.style.display='none';return;}hoverActive=true;
    const cross=svg.querySelector('#cross');cross.setAttribute('x1',sx);cross.setAttribute('x2',sx);cross.setAttribute('visibility','visible');svg.querySelectorAll('[data-sym]').forEach(l=>{const on=l.dataset.sym===best.x.symbol;l.setAttribute('opacity',on?1:.16);l.setAttribute('stroke-width',on?5:l.dataset.w);});const dot=svg.querySelector('#dot');dot.setAttribute('cx',X(best.q.t));dot.setAttribute('cy',best.y);dot.setAttribute('visibility','visible');const rangeText=`${best.x.totalRet>=0?'+':''}${(best.x.totalRet*100).toFixed(2)}%`;tip.innerHTML=`<b>${best.x.symbol}</b><div class="muted">${names[best.x.symbol]||best.x.symbol}</div><div>Rank: <b>#${rank.findIndex(r=>r.symbol===best.x.symbol)+1}</b></div><div>Price: <b>$${best.q.price.toFixed(2)}</b></div><div>${range} Total Change: <b>${rangeText}</b></div><div>${new Date(best.q.t).toLocaleString()}${best.q.live?' · LIVE IEX':''}</div>`;tip.style.display='block';tip.style.left=`${Math.min(rect.width-240,e.clientX-rect.left+12)}px`;tip.style.top=`${Math.max(8,e.clientY-rect.top-20)}px`;
  });
  ...
  async function loadToday(force=false){if(todayLoading)return;if(!force&&todayData.size&&Date.now()-todayLoadedAt<60000){render();return;}todayLoading=true;if(!hoverActive)render();try{const response=await fetch(`/api/intraday-24h-top10?t=${Date.now()}`,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const payload=await response.json();todayData=new Map(Object.entries(payload.series||{}).map(([symbol,rows])=>[symbol,rows.map(x=>({t:Date.parse(x.t),price:Number(x.price)})).filter(x=>Number.isFinite(x.t)&&Number.isFinite(x.price)).sort((a,b)=>a.t-b.t)]));todayLoadedAt=Date.now();}catch(err){console.warn('[TODAY INTRADAY]',err);}finally{todayLoading=false;if(hoverActive)pendingRender=true;else render();}}
  card.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.r){range=b.dataset.r;zoom=1;pan=1;if(range==='TODAY'){loadToday();return;}}else if(b.dataset.m)mode=b.dataset.m;else if(b.dataset.s){hidden.has(b.dataset.s)?hidden.delete(b.dataset.s):hidden.add(b.dataset.s);}else if(b.dataset.a){if(b.dataset.a==='in')zoom=Math.min(8,zoom*1.5);if(b.dataset.a==='out')zoom=Math.max(1,zoom/1.5);if(b.dataset.a==='left')pan=Math.max(0,pan-.2);if(b.dataset.a==='right')pan=Math.min(1,pan+.2);if(b.dataset.a==='reset'){zoom=1;pan=1;}}hoverActive=false;render();});
  window.addEventListener('ds:all-live-stock-quotes',e=>{quotes=e.detail?.quotes||{};if(hoverActive){pendingRender=true;return;}if(range==='TODAY'&&Date.now()-todayLoadedAt>60000&&!todayLoading)loadToday(true);else if(data.size||todayData.size)render();});
  async function load(){try{const response=await fetch('/api/local-history-bulk?limit=1300'),payload=await response.json();data=new Map(Object.entries(payload.series||{}).map(([symbol,rows])=>[symbol,rows.map(x=>({t:Date.parse(x.t),price:Number(x.price)})).filter(x=>Number.isFinite(x.t)&&Number.isFinite(x.price))]));render();}catch(err){console.warn(err);stamp.textContent='HISTORY UNAVAILABLE';rankList.innerHTML='<div class="muted">Ranking history unavailable.</div>';}}
