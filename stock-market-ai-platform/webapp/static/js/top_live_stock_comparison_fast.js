(()=>{
  ...
  let range='90D',mode='growth',zoom=1,pan=1,displayCount=10,hidden=new Set(),geo=null,hoverActive=false,pendingRender=false;
  ...
  function ranking(){const source=range==='TODAY'?todayData:data;return [...source].map(([symbol,baseRows])=>{const histRows=data.get(symbol)||baseRows,fullRows=selectedRangeRows(symbol,histRows),rows=rowsFor(symbol,histRows),totalRet=fullRows.length>1?fullRows.at(-1).price/fullRows[0].price-1:-Infinity;return{symbol,rows,totalRet,ret:totalRet};}).filter(x=>x.rows.length>1&&Number.isFinite(x.totalRet)).sort((a,b)=>b.totalRet-a.totalRet);}

  function renderRankingList(rank){rankRange.textContent=`${range==='TODAY'?'TODAY':range} · ${rank.length} STOCKS`;if(!rank.length){rankList.innerHTML=`<div class="muted">${range==='TODAY'&&todayLoading?'Loading market session…':'No ranked stocks available.'}</div>`;return;}const maxMagnitude=Math.max(...rank.map(x=>Math.abs(x.totalRet)),0.0001);rankList.innerHTML=rank.map((x,i)=>{const pct=x.totalRet*100,width=Math.max(7,Math.min(100,Math.abs(x.totalRet)/maxMagnitude*100)),company=names[x.symbol]||x.symbol;return `<div class="tlrank-row ${i<3?'top3':''}" data-rank-symbol="${x.symbol}"><div class="tlrank-badge">#${i+1}</div><div><div class="tlrank-symbol">${x.symbol}</div><div class="tlrank-company" title="${company}">${company}</div><div class="tlrank-track"><div class="tlrank-fill" style="width:${width.toFixed(1)}%"></div></div></div><div><div class="tlrank-caption">${range==='TODAY'?'TODAY':'TOTAL'} CHANGE</div><div class="tlrank-return ${pct>=0?'pos':'neg'}">${pct>=0?'+':''}${pct.toFixed(2)}%</div></div></div>`;}).join('');}

  function render(){
    svg.innerHTML='';tip.style.display='none';const universeRank=ranking();renderRankingList(universeRank);card.querySelectorAll('[data-count]').forEach(b=>b.classList.toggle('on',Number(b.dataset.count)===displayCount));const rank=universeRank.slice(0,Math.min(displayCount,universeRank.length));card.querySelectorAll('[data-r]').forEach(b=>b.classList.toggle('on',b.dataset.r===range));card.querySelectorAll('[data-m]').forEach(b=>b.classList.toggle('on',b.dataset.m===mode));legend.innerHTML='<b class="muted">LINES</b>'+rank.map((x,i)=>`<button data-s="${x.symbol}" class="${hidden.has(x.symbol)?'off':''}"><i style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${colors[i]};margin-right:5px"></i>${i+1}. ${x.symbol} ${x.totalRet>=0?'+':''}${(x.totalRet*100).toFixed(2)}%</button>`).join('');
  ...
  card.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.count){displayCount=Number(b.dataset.count)||10;hidden.clear();hoverActive=false;render();return;}if(b.dataset.r){range=b.dataset.r;zoom=1;pan=1;if(range==='TODAY'){loadToday();return;}if(range==='ALL'&&dataLimit<2600){load(2600);return;}}else if(b.dataset.m)mode=b.dataset.m;else if(b.dataset.s){hidden.has(b.dataset.s)?hidden.delete(b.dataset.s):hidden.add(b.dataset.s);}else if(b.dataset.a){if(b.dataset.a==='in')zoom=Math.min(8,zoom*1.5);if(b.dataset.a==='out')zoom=Math.max(1,zoom/1.5);if(b.dataset.a==='left')pan=Math.max(0,pan-.2);if(b.dataset.a==='right')pan=Math.min(1,pan+.2);if(b.dataset.a==='reset'){zoom=1;pan=1;}}hoverActive=false;render();});
