(()=>{
  ...
  let data=new Map(),todayData=new Map(),todayLoading=false,todayLoadedAt=0,quotes={},dataLimit=0,historyLoading=false;
  ...
  function appendLive(symbol,rows){const out=[...rows],live=Number(quotes[symbol]?.reference_price);if(out.length&&Number.isFinite(live)&&live>0){const liveTs=Date.parse(quotes[symbol]?.timestamp||'')||Date.now(),last=out.at(-1);if(!last||Math.abs(last.t-liveTs)>500)out.push({t:liveTs,price:live,live:true});else out[out.length-1]={t:liveTs,price:live,live:true};}return out;}
  ...
  function render(){
    ...
    E('line',{id:'cross',y1:p.t,y2:H-p.b,stroke:'#f2f6ff','stroke-dasharray':'5 4',visibility:'hidden'});E('circle',{id:'dot',r:5.5,fill:'#fff',stroke:'#07101f','stroke-width':2,visibility:'hidden'});geo={W,H,p,t0,td,X,Y,shown:plotted,rank};stamp.textContent=`${range==='TODAY'?'TODAY':range} · LIVE`;card.querySelector('#tlz').textContent=zoom===1?'FULL RANGE':`${zoom.toFixed(1)}× ZOOM`;
    note.textContent=range==='TODAY'
      ?'TODAY = the latest available U.S. market session from the 9:30 AM ET open through the newest cached/live IEX observation.'
      :longRanges.has(range)
        ...
        :'Short historical views align to the first timestamp where all displayed lines have data; the newest cached/live IEX observation is appended when newer than persisted EOD history.';
  }
