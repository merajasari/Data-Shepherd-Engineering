...
class CryptoDashboardContractTest(unittest.TestCase):
    def test_dashboard_exposes_only_retained_forward_sections(self):
        ...
        self.assertIn("September 23, 2026 07:00 UTC observation boundary", template)
        self.assertIn("first admissible decision is September 24, 2026 00:00 UTC", template)
        self.assertIn("Only completed three-day realizations move paper equity", template)
        self.assertIn("Next realization eligible", template)
        self.assertIn("Horizon:</strong> {{ v5.horizon_days }} days", template)
    ...
    def test_shared_v4_actions_summary_reuses_drilldown_evidence_and_defaults_to_24h(self):
        ...
        self.assertIn("Full hourly drill-in", browser)
