...
if [[ ! -f "$PROJECT/ml/crypto_xrp_v1/forward_service.py" ]]; then
  echo "XRP forward service not found." >&2
  exit 1
fi
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>ml.crypto_xrp_v1.forward_service</string>
...
# Remove an older loaded copy if present. Ignore "not found" failures.
...
echo "stderr: $ERR_LOG"
echo
echo "The service is shadow-only and places no brokerage orders."
